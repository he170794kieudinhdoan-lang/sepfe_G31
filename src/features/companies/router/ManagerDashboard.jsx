...
const MOCK_COMPANIES = [
  {
    ...
    description: 'Công ty chuyên phát triển nền tảng tuyển dụng và HR Tech. Hoạt động trong lĩnh vực công nghệ thông tin.',
...
const MOCK_PENDING_COMPANIES = [
  { id: 2, companyName: 'LogiFast Việt Nam', owner: 'loan@logifast.vn', submittedDate: '2025-02-10', status: 'Pending' },
];
...
// ========================
const statusConfig = {
  APPROVED: { label: 'Đã duyệt', variant: 'default', className: 'bg-emerald-100 text-emerald-700 border-0' },
  PENDING: { label: 'Chờ duyệt', variant: 'secondary', className: 'bg-amber-100 text-amber-700 border-0' },
  REJECTED: { label: 'Từ chối', variant: 'destructive', className: 'bg-red-100 text-red-700 border-0' },
  DELETED: { label: 'Đã xoá', variant: 'secondary', className: 'bg-gray-100 text-gray-500 border-0' },
};
...
const CompanyDetail = ({ company, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      <Button variant="outline" className="rounded-xl" onClick={onBack}>←</Button>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">{company.address || 'Chưa có địa chỉ'}</p>
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4 rounded-xl shadow-sm border-0">
          ...
          <p className="text-sm font-medium mt-1">
            {company.businessLicenseUrl ? (
              <a href={company.businessLicenseUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
        ...
        <Card className="p-4 rounded-xl shadow-sm border-0">
          ...
          <p className="text-sm font-medium mt-1">
            {company.logoUrl ? (
              <a href={company.logoUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
            ...
            <div key={i} className="flex items-start py-3 gap-3">
              ...
              <div className="flex-1 text-sm">
                {f.isStatus ? (
                  ...
                  <a href={f.value} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
...
export const ManagerDashboard = () => {
  ...
  const handleDisableJob = () => { setDisableJobConfirm(false); setSelectedReport(null); toast(MSG.MSG37); };
  const handleEnableJob = () => { setEnableJobConfirm(false); toast(MSG.MSG37); };
  const handleResolve = () => { setSelectedReport(null); toast(MSG.MSG37); };
  const handleApproveCompany = () => { setSelectedApproval(null); setApproveNote(''); toast(MSG.MSG55); };
  const handleRejectCompany = () => { setSelectedApproval(null); setRejectNote(''); toast(MSG.MSG54, 'error'); };
          ...
          <div className="space-y-6 ">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              <Card className="p-6 lg:col-span-2 rounded-xl shadow-sm border-0">
                ...
                <div className="h-60 rounded-xl bg-gradient-to-br from-blue-50 via-white to-amber-50 border border-dashed flex items-center justify-center text-muted-foreground">
            ...
            <div className="flex flex-wrap gap-3">
              <Button variant="outline" className="rounded-xl" onClick={() => setActive('companies')}>
            ...
            {selectedCompany ? (
              ...
              <>
                ...
                <Card className="p-4 rounded-xl shadow-sm border-0 flex flex-wrap gap-3 items-center">
                  ...
                  <Button variant="outline" className="rounded-xl" onClick={() => { setCompanyFilter(''); setCompanyStatusFilter(''); }}>
                    Reset
                  </Button>
                  <span className="text-xs text-muted-foreground ml-auto">{filteredCompanies.length} công ty</span>
                </Card>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filteredCompanies.map((c) => (
                        <tr key={c.id} className="border-b last:border-b-0 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">{c.address?.split(',').slice(-1)[0]?.trim() || '—'}</p>
                              </div>
                            </div>
                          </td>
            ...
            {pendingCompanies.length === 0 ? (
              ...
              <Card className="p-6 rounded-xl shadow-sm border-0 max-w-2xl">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <Button variant="outline" className="rounded-xl" onClick={() => setSelectedApproval(null)}>Quay lại</Button>
                </div>
              </Card>
            ) : (
                  ...
                  <tbody>
                    {pendingCompanies.map((c) => (
                      <tr key={c.id} className="border-b last:border-b-0">
                        ...
                        <td>
                          <Button variant="outline" size="sm" className="rounded-xl" onClick={() => setSelectedApproval(c)}>Xem</Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
