...
import { MapPin, Trash2, Wallet, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
    ...
    return (
      <div className="bg-gray-50 min-h-full py-12">
        <div className="container mx-auto px-4">
          ...
          <EmptyState title="Bạn chưa đăng nhập" description="Đăng nhập để xem danh sách công việc đã lưu." actionLabel="Đăng nhập ngay" actionLink="/auth/login" />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-[#fbfa\f9] min-h-full py-12">
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <EmptyState title="Chưa có việc làm nào được lưu" description="Không tìm thấy việc làm trong danh sách đã lưu của bạn." actionLabel="Xem việc làm bổ sung" />
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="space-y-4">
          {list.map((item) => {
            ...
            const locationStr = job.province ? `${job.district ? job.district + ', ' : ''}${job.province}` : 'Toàn quốc';

            const formatMoney = (number) => number ? number.toLocaleString('vi-VN') + ' đ' : null;
            const salaryStr = job.salaryMin && job.salaryMax
              ? `${formatMoney(job.salaryMin)} - ${formatMoney(job.salaryMax)}`
              : (job.salaryMax ? formatMoney(job.salaryMax) : 'Thỏa thuận');

            return (
              <Card key={item.id} className="group p-4 rounded-2xl shadow-sm border border-gray-100 bg-white flex flex-col sm:flex-row gap-4 hover:shadow-md hover:border-primary/40 transition-all duration-300">
                {company.logoUrl ? (
                  <div className="w-full sm:w-20 h-24 sm:h-20 rounded-xl overflow-hidden shrink-0 border border-gray-100 p-1.5 shadow-sm bg-white group-hover:border-primary/20 transition-colors">
                    <ImageWithFallback src={company.logoUrl} alt={company.name} className="w-full h-full object-contain" fallbackClassName="w-full h-full bg-primary/5" />
                  </div>
                ) : (
                  <div className="w-full sm:w-20 h-24 sm:h-20 rounded-xl bg-primary/5 flex items-center justify-center shrink-0 border border-primary/10">
                    <span className="text-primary font-bold text-xl">{company.name?.[0]?.toUpperCase()}</span>
                  </div>
                )}
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  ...
                  <p className="text-sm font-medium text-gray-600 line-clamp-1 mt-0.5">{company.name}</p>

                  <div className="flex flex-wrap items-center gap-3 mt-2.5 text-xs text-gray-500">
                    <span className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-md">
                      <MapPin className="h-3.5 w-3.5 text-primary" /> {locationStr}
                    </span>
                ...
                <div className="flex flex-row sm:flex-col items-center sm:items-end justify-center gap-2 shrink-0 mt-3 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button variant="outline" size="sm" className="rounded-full flex-1 sm:flex-none h-8 px-4 text-xs font-medium hover:border-primary hover:text-primary transition-colors" asChild>
                      <Link to={`/job/${job.id}`}>Chi tiết</Link>
                    </Button>
                    <Button variant="default" size="sm" className="rounded-full flex-1 sm:flex-none h-8 px-4 text-xs bg-primary text-primary-foreground hover:bg-primary/90 font-medium shadow-sm transition-transform active:scale-95" asChild>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 mt-10">
            <Button variant="outline" size="icon" className="rounded-full border-gray-200 hover:border-primary hover:text-primary transition-colors" disabled={page <= 1} onClick={() => setPage(page - 1)}>
            ...
            <div className="flex items-center px-4 h-10 rounded-full bg-white border border-gray-200 shadow-sm text-sm font-medium">
              <span className="text-primary mr-1">{page}</span> / <span className="text-gray-500 ml-1">{totalPages}</span>
            </div>
            <Button variant="outline" size="icon" className="rounded-full border-gray-200 hover:border-primary hover:text-primary transition-colors" disabled={page >= totalPages} onClick={() => setPage(page + 1)}>
