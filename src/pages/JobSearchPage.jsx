...
const formatSalary = (min, max) => {
    const f = (v) => {
        if (v >= 1000000) return `${(v / 1000000).toFixed(v % 1000000 === 0 ? 0 : 1)} triệu`;
...
const shiftLabel = (shift) => WORKING_SHIFTS.find((s) => s.value === shift)?.label || shift;
const genderLabel = (g) => {
...
export const JobSearchPage = () => {
    ...
    const [genderRequirement, setGenderRequirement] = useState(searchParams.get('genderRequirement') || '');
    const [salaryRange, setSalaryRange] = useState(searchParams.get('salaryRange') || '');
    ...
    }, [keyword, province, workingShift, occupationId, companyId, genderRequirement, salaryRange, sortBy, page, setSearchParams]);
    ...
    // ========================
    return (
        <div className="bg-gradient-to-b from-amber-50/50 via-white to-gray-50 min-h-screen">
            {/* Hero Search Bar */}
                    ...
                    <main className="flex-1 min-w-0">
                        ...
                        {isLoading ? (
                            ...
                            <Card className="p-8 rounded-2xl text-center border-0 shadow-sm">
                                <div className="text-destructive text-lg font-semibold mb-2">Không thể tải dữ liệu</div>
                                <p className="text-sm text-muted-foreground mb-4">Vui lòng kiểm tra kết nối và thử lại.</p>
                ...
                <>
                    ...
                    <div className="fixed left-0 top-0 bottom-0 w-80 bg-white/95 backdrop-blur-md p-5 z-50 shadow-2xl lg:hidden overflow-y-auto animate-in slide-in-from-left duration-300">
                        <div className="flex justify-between items-center mb-5">
                            ...
                            <Button variant="ghost" size="sm" className="rounded-xl" onClick={() => setDrawerOpen(false)}>
                                <X className="h-4 w-4" />
                            </Button>
                        </div>
